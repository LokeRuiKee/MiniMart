...
while True:
    ...
    # Encode frame to JPEG and save
    _, jpeg_frame = cv2.imencode('.jpg', annotated_frame)

    f=open("static/output.jpg", "wb")
    f.write(jpeg_frame.tobytes())

    # Optional: Display the annotated frame in a window (for debugging)
    cv2.imshow("Website Integration Test", annotated_frame)
    
    # Break the loop on 'q' key press
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

    time.sleep(0.1)  # Adjust delay for desired frame rate
